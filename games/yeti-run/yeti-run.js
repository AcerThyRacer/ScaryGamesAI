...
(function () {
    ...
    // Fixed-timestep simulation (decouple gameplay from render FPS)
    var fixedStep = 1 / 60;
    var accumulator = 0;
    var maxSubSteps = 8;

    // Render interpolation (temporary overrides during render only)
    var prevPlayerPos = null, prevYetiPos = null, prevCamPos = null;
    var simPlayerPos = null, simYetiPos = null, simCamPos = null;
    ...
    GameUtils.initPause({
        onResume: function () {
            ...
            if (clock) clock.getDelta(); // prevent a large first frame dt
            accumulator = 0;
            animate();
        },
        ...
        setTimeout(function () {
            ...
            setTimeout(function () {
                ...
                ctrl.classList.remove('hiding');
                 ...
                 obstacles = [];
                 clock.getDelta(); // prevent a large first frame dt
                 accumulator = 0;
                 animate();
             }, 800);
         }, 3500);
    ...
    function updateYeti(dt) {
        ...
        // NOTE: visuals (limb animation, sway, breath fog, bob) are handled in the render-rate pass.
    ...
    // Render-rate visual animation pass (runs at display refresh rate)
    function updateCharacterAnimations() {
        var now = Date.now();

        if (player && player.userData) {
            var runCycleP = Math.sin(now * 0.012) * 0.4;
            if (player.userData.leftLeg) player.userData.leftLeg.rotation.x = runCycleP;
            if (player.userData.rightLeg) player.userData.rightLeg.rotation.x = -runCycleP;
            if (player.userData.leftArm) player.userData.leftArm.rotation.x = -runCycleP * 0.6;
            if (player.userData.rightArm) player.userData.rightArm.rotation.x = runCycleP * 0.6;
        }

        if (yeti && yeti.userData) {
            // Bob and sway are visual-only.
            yeti.position.y = Math.sin(now * 0.003) * 0.3;
            yeti.rotation.y = Math.sin(now * 0.004) * 0.1;

            var runCycleY = Math.sin(now * 0.008) * 0.6;
            if (yeti.userData.leftLeg) yeti.userData.leftLeg.rotation.x = runCycleY;
            if (yeti.userData.rightLeg) yeti.userData.rightLeg.rotation.x = -runCycleY;
            if (yeti.userData.leftArm) yeti.userData.leftArm.rotation.x = -runCycleY * 0.8;
            if (yeti.userData.rightArm) yeti.userData.rightArm.rotation.x = runCycleY * 0.8;

            if (breathFog) {
                breathFog.intensity = 0.3 + Math.sin(now * 0.005) * 0.3;
            }
        }
    }
    ...
    function animate() {
        ...
        while (accumulator >= fixedStep && subSteps < maxSubSteps) {
            // Capture previous sim state for interpolation.
            if (player && player.position) {
                if (!prevPlayerPos) prevPlayerPos = new THREE.Vector3();
                prevPlayerPos.copy(player.position);
            }
            if (yeti && yeti.position) {
                if (!prevYetiPos) prevYetiPos = new THREE.Vector3();
                prevYetiPos.copy(yeti.position);
            }
            ...
            updateYeti(fixedStep);
            updateCamera();
            updateObstacles(fixedStep);
            updateSnow(fixedStep);
            recycleEnvironment();
        ...
        // Interpolate between last and current sim states for smooth rendering.
        renderInterpActive = false;
        var alpha = fixedStep > 0 ? (accumulator / fixedStep) : 0;
        if (alpha < 0) alpha = 0;
        if (alpha > 1) alpha = 1;

        if (prevPlayerPos && player && player.position) {
            if (!simPlayerPos) simPlayerPos = new THREE.Vector3();
            simPlayerPos.copy(player.position);
            player.position.lerpVectors(prevPlayerPos, simPlayerPos, alpha);
            renderInterpActive = true;
        }
        if (prevYetiPos && yeti && yeti.position) {
            if (!simYetiPos) simYetiPos = new THREE.Vector3();
            simYetiPos.copy(yeti.position);
            yeti.position.lerpVectors(prevYetiPos, simYetiPos, alpha);
            renderInterpActive = true;
        }
        if (prevCamPos && camera && camera.position) {
        ...
        if (renderInterpActive && camera && player) {
            camera.lookAt(player.position.x, player.position.y + 2, player.position.z - 15);
        }
        ...
        // Restore sim transforms after rendering.
        if (renderInterpActive) {
            if (simPlayerPos && player && player.position) player.position.copy(simPlayerPos);
            if (simYetiPos && yeti && yeti.position) yeti.position.copy(simYetiPos);
            if (simCamPos && camera && camera.position) camera.position.copy(simCamPos);
        }
    }
})();
