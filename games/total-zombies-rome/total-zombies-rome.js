/* Total Zombies: Rome — Enhanced 3D Total War RTS (prequel) */
(function () {
    'use strict';
    ...
    var UNIT_ROLE_BY_TYPE = {
        ...
        // Legacy aliases (in case levels reuse old ids)
        archers: 'archers',
        archer: 'archers',
        cavalry: 'cavalry',
        knight: 'cavalry',
        ...
        // Undead
        'zombie-gaul': 'swordsmen',
        'zombie-shambler': 'swordsmen',
        'zombie-runner': 'swordsmen',
        'zombie-brute': 'swordsmen',
        'zombie-necromancer': 'archers',
        'zombie-spitter': 'archers',
    };

    function getUnitRole(type) {
        return UNIT_ROLE_BY_TYPE[type] || 'swordsmen';
    }

    function getUnitStats(team, type) {
        var role = getUnitRole(type);
        var stats = role === 'cavalry'
            ? { hp: 140, atk: 20, range: 1.8, speed: 7, atkCooldown: 0.9 }
            : role === 'archers'
                ? { hp: 80, atk: 10, range: 14, speed: 3, atkCooldown: 1.6 }
                : { hp: 100, atk: 14, range: 1.8, speed: 3.8, atkCooldown: 0.9 };

        if (team === 'blue') {
            // Roman tuning: disciplined infantry, fragile skirmishers, fast cavalry
            if (type === 'legionary') stats = { hp: 120, atk: 15, range: 1.85, speed: 3.7, atkCooldown: 0.88 };
            else if (type === 'centurion') stats = { hp: 150, atk: 20, range: 1.9, speed: 3.8, atkCooldown: 0.82 };
            else if (type === 'velites') stats = { hp: 85, atk: 11, range: 13.5, speed: 3.3, atkCooldown: 1.45 };
            else if (type === 'equites') stats = { hp: 150, atk: 22, range: 1.9, speed: 7.2, atkCooldown: 0.9 };
            else if (type === 'scorpio') stats = { hp: 95, atk: 16, range: 17, speed: 2.6, atkCooldown: 2.1 };
        } else if (team === 'red') {
            if (type === 'zombie-runner') stats = { hp: 78, atk: 12, range: 1.6, speed: 5.3, atkCooldown: 0.75 };
            else if (type === 'zombie-brute') stats = { hp: 190, atk: 28, range: 1.9, speed: 2.6, atkCooldown: 1.25 };
            else if (type === 'zombie-necromancer' || type === 'zombie-spitter') stats = { hp: 92, atk: 14, range: 16, speed: 2.9, atkCooldown: 1.7 };
            else if (type === 'zombie-gaul') stats = { hp: 110, atk: 15, range: 1.85, speed: 3.9, atkCooldown: 0.95 };
        }
    ...
    function Unit(team, type, x, z) {
        this.team = team; this.type = type; this.role = getUnitRole(type); this.x = x; this.z = z; this.y = 0;
        this.tx = x; this.tz = z; this.facing = team === 'blue' ? 0 : Math.PI;
        this.targetFacing = this.facing;
        var stats = getUnitStats(team, type);
        this.hp = stats.hp;
        this.maxHp = this.hp;
        this.atk = stats.atk;
        this.range = stats.range;
        this.speed = stats.speed;
        this.atkCooldown = stats.atkCooldown;
        ...
        // Rome perks
        this.pilumCd = (team === 'blue' && (type === 'legionary' || type === 'centurion')) ? 0.6 + Math.random() * 0.8 : 9999;
        // Debuffs
        this.slowT = 0;
        // Animation state
    ...
    function createUnitMesh(u) {
        ...
        var role = u.role || getUnitRole(u.type);
        // Rome palette: red tunics and bronze; undead are sickly greens/browns.
        var c = isBlue ? 0xaa2222 : 0xcc3333;
        var dc = isBlue ? 0x661111 : 0x882222;
        var skinColor = isBlue ? 0xddbb99 : 0x557755;
        if (!isBlue) {
            if (u.type === 'zombie-runner') { c = 0x6a8f4a; dc = 0x4f6e35; skinColor = 0x6f9960; }
            else if (u.type === 'zombie-brute') { c = 0x70483a; dc = 0x503126; skinColor = 0x5f6a4d; }
            else if (u.type === 'zombie-necromancer' || u.type === 'zombie-spitter') { c = 0x3f7a58; dc = 0x2b5a40; skinColor = 0x6ea776; }
            else if (u.type === 'zombie-gaul') { c = 0x5f7a46; dc = 0x3c4f2e; skinColor = 0x6a8a58; }
        }
        ...
        var bodyMat;
        if (isBlue) bodyMat = (role === 'archers') ? leatherMat : steelMat;
        ...
        if (isBlue && role !== 'archers') {
            var crest = new THREE.Mesh(new THREE.BoxGeometry(0.15, 0.15, 0.02),
                new THREE.MeshStandardMaterial({ color: 0xccaa44, metalness: 0.55, roughness: 0.25 }));
            crest.position.set(0, 0.82, 0.16); g.add(crest);
        }
        // Shoulder pauldrons for blue
        if (isBlue) {
            var pMat = role === 'archers' ? leatherMat : steelMat;
        ...
        if (isBlue) {
            if (role === 'swordsmen') {
                // Roman galea + crest
                var helm = new THREE.Mesh(new THREE.SphereGeometry(0.22, 10, 8), steelMat);
                helm.position.y = 1.30; g.add(helm);
                var brow = new THREE.Mesh(new THREE.BoxGeometry(0.28, 0.06, 0.02),
                    new THREE.MeshStandardMaterial({ color: 0xccaa44, metalness: 0.7, roughness: 0.3 }));
                brow.position.set(0, 1.26, 0.2); g.add(brow);
                var crest = new THREE.Mesh(new THREE.BoxGeometry(0.06, 0.22, 0.30),
                    new THREE.MeshStandardMaterial({ color: 0xaa2222, roughness: 0.85, metalness: 0.05 }));
                crest.position.set(0, 1.50, -0.02); g.add(crest);
            } else if (role === 'archers') {
                // Leather cap
        ...
        var armMat = isBlue ? (role === 'archers' ? leatherMat : chainMat) : new THREE.MeshStandardMaterial({ color: c, roughness: 0.7 });
        ...
        if (role === 'swordsmen') {
            ...
            // Roman scutum (tall rectangle) for blue
            var shieldW = isBlue ? 0.06 : 0.04, shieldH = isBlue ? 0.72 : 0.45, shieldD = isBlue ? 0.46 : 0.35;
            ...
            if (isBlue) {
                var bard = new THREE.Mesh(new THREE.BoxGeometry(0.6, 0.3, 1.15),
                    new THREE.MeshStandardMaterial({ color: 0xaa2222, roughness: 0.6, metalness: 0.15 }));
        ...
        if (!isBlue) {
            if (u.type === 'zombie-runner') g.scale.set(0.9, 0.9, 0.9);
            else if (u.type === 'zombie-brute') g.scale.set(1.15, 1.2, 1.15);
            else if (u.type === 'zombie-necromancer') g.scale.set(0.98, 1.05, 0.98);
        }
    ...
    function updateSelPanel() {
        ...
        selectedUnits.forEach(function (u) { var role = u.role || getUnitRole(u.type); c[role]++; th += u.hp; tm += u.maxHp; });
    ...
    function updateUnits(dt) {
        allUnits.forEach(function (u) {
            ...
            if (u.pilumCd < 9999) u.pilumCd = Math.max(0, u.pilumCd - dt);
            // Auto-acquire
            ...
            if (u.state === 'move') {
                ...
                var spd = u.speed * (u.slowT > 0 ? 0.65 : 1);
                var s = Math.min(spd * dt, dist); u.x += (dx / dist) * s; u.z += (dz / dist) * s;
                ...
                if (dist > u.range + 0.5 && !u.guarding) {
                    var spdA = u.speed * (u.slowT > 0 ? 0.65 : 1);
                    u.x += (dx / dist) * spdA * dt; u.z += (dz / dist) * spdA * dt;
                }
                else {
                    u.atkTimer -= dt;

                    // Roman pilum volley: disciplined infantry throws before melee contact.
                    if (u.team === 'blue' && (u.type === 'legionary' || u.type === 'centurion') && u.pilumCd <= 0) {
                        if (dist > 3.2 && dist < 10.5) {
                            u.pilumCd = 5.2 + Math.random() * 1.2;
                            var pdmg = u.atk * 0.9 * (0.85 + Math.random() * 0.25);
                            u.target.hp -= pdmg;
                            u.target.slowT = Math.max(u.target.slowT || 0, 2.4);
                            spawnPilum(u, u.target);
                    ...
                    if (u.atkTimer <= 0) {
                        ...
                        if (u.role === 'archers') spawnArrow(u, u.target);
                    }
                }
            } else {
                // Idle weapon reset
    ...
    function spawnArrow(from, to) {
        ...
        g.position.set(from.x, 1.5, from.z); scene.add(g); arrows.push({ mesh: g, tx: to.x, ty: 0.8, tz: to.z, life: 0.8, ttl: 0.8 });
    }
    function spawnPilum(from, to) {
        var g = new THREE.Mesh(new THREE.CylinderGeometry(0.022, 0.022, 0.9), new THREE.MeshBasicMaterial({ color: 0xffcc66 }));
        g.position.set(from.x, 1.55, from.z);
        scene.add(g);
        arrows.push({ mesh: g, tx: to.x, ty: 0.9, tz: to.z, life: 0.9, ttl: 0.9 });
    }
    function updateArrows(dt) {
        for (var i = arrows.length - 1; i >= 0; i--) {
            var a = arrows[i]; a.life -= dt; var ttl = a.ttl || 0.8; var t = 1 - a.life / ttl;
    ...
    document.addEventListener('keydown', function (e) {
        ...
        if (e.code === 'Tab') { e.preventDefault(); var squadCount = Math.max(1, blueUnits.reduce(function (m, u) { return Math.max(m, u.squadId); }, 0) + 1); currentSquadIdx = (currentSquadIdx + 1) % squadCount; clearSelection(); blueUnits.forEach(function (u) { if (!u.dead && u.squadId === currentSquadIdx) selectUnit(u, true); }); }
